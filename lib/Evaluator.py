...
class Evaluator:
    def get_pascalVOC_metrics(self, boundingboxes, IOU_threshold=0.5):
    ...
    def plot_precision_recall_curve(self,
        ...
        Args:
            class_id: The class that will be plot;
            ...
            show_graphics (optional): if True, the plot will be shown (default = True)
        Returns:
            ...
            dict['total FP']: total number of False Negative detections;
        """
        results = self.get_pascalVOC_metrics(bounding_boxes, IOUThreshold)
        result = None
        for res in results:
            if res['class'] == class_id:
                ...
                break
        if result is None:
            raise IOError('Error: Class %d could not be found.' % class_id)
